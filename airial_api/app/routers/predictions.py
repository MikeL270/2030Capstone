...
def get_predictions(query: PredictionQuery):
	...
	'''
	try:
		predictions = base.get_predictions(query, cast(User, current_user))
...
def get_prediction_by_id(prediction_id: str):
	...
	'''
	try:
		prediciton = base.get_prediction(UUID(prediction_id), cast(User, current_user))
	except AuthorizationFailure as e:
		abort(401, str(e))
...
def create_prediction(body: CreatePredictionReq):
	...
	'''
	try:
		prediction = base.create_prediction(body, cast(User, current_user))
	except AuthorizationFailure as e:
		abort(401, str(e))
