...
<template>
  <nav class="primary-nav vh-100 bg-body-secondary" @mouseenter="sStore.toggle_nav(true)"
    @mouseleave="sStore.toggle_nav(false)" :style="{
    ...
    <BNav vertical pills fill class="w-100 h-100 p-2">
      <BNavItem to="/" :active="$route.path === '/'">
        ...
        <span v-if="sStore.nav_toggled" class="ms-3 d-none d-md-inline">Dashboard</span>
      </BNavItem>
      ...
      <BNavItem v-if="sStore.is_admin" to="/upload" :active="$route.path.startsWith('/upload')">
      ...
      <BNavItem>
        ...
        <span v-if="sStore.nav_toggled" class="ms-3 d-none d-md-inline">Data Manager</span>
      </BNavItem>
