...
export default defineComponent({
  ...
  watch: {
    CurrentPredictionCrops(
      newValue: PredictionCrop[],
      oldValue: PredictionCrop[],
    ) {
  ...
  methods: {
    ...
    drawBoundingBox(canvas: HTMLCanvasElement, predCrop: PredictionCrop) {
      ...
      ctx.beginPath();
      ctx.lineWidth = 2;
      const label_color_hex = this.pStore.labels?.find(
        (label) => label.label == predCrop.label,
      )?.color;
      ctx.strokeStyle =
        label_color_hex != undefined ? label_color_hex : "white";
      ctx.fillStyle =
    ...
    async handleLeftBracket() {
      ...
      if (this.cStore.CurrentPredictionCrop) {
        this.predictionRefs[
          this.cStore.CurrentPredictionCrop.uuid
    ...
    handleS() {
      if (this.cStore.CurrentPredictionCrop) {
        this.cStore.CurrentPredictionCrop.approved = this.cStore
          .CurrentPredictionCrop.approved
          ? false
          : true;
        this.drawBoundingBox(
          this.predCropRefs[this.cStore.CurrentPredictionCrop.uuid],
          this.cStore.CurrentPredictionCrop,
        );
      }
    },
    async handleEnter() {
    ...
    decodeDigit(event: KeyboardEvent) {
      ...
      if (this.cStore.CurrentPredictionCrop) {
        this.cStore.CurrentPredictionCrop.label =
          this.pStore.labels?.find((label) => label.label == label_num) !=
          undefined
            ? label_num
            : this.cStore.CurrentPredictionCrop.label;
        this.drawBoundingBox(
  ...
  <div v-if="!cStore.loading" class="d-flex h-100 flex-column">
    <BContainer fluid class="h-100">
      <BRow class="h-100 d-flex justify-content-center">
        <div
          id="predictionsCarosuel"
          v-if="!cStore.loading"
          :class="{ Overflow: cStore.CurrentPredictionCrops.length > 2 }"
        >
          <div
            v-for="predCrop in cStore.CurrentPredictionCrops"
            :key="predCrop.uuid"
            class="predictionObject bg-body-secondary rounded-3 shadow"
            :class="{
              Approved: predCrop.approved == true,
              Selected:
                cStore.activePredIdx ==
            ...
            :ref="
              (el) => {
                if (el) {
                  predictionRefs[predCrop.uuid] = el as HTMLDivElement;
                }
              }
            "
          >
            <span class="fs-4"> Score: {{ predCrop.score?.toFixed(3) }} </span>
            <button
              @click="predCrop.approved = predCrop.approved ? false : true"
              tabindex="-1"
            >
              <canvas
                :ref="
            ...
            <div
              class="d-flex w-100 h-100 m-1 p-1 justify-content-evenly align-items-center"
            >
