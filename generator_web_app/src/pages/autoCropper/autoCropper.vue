  ...
  <div v-if="!cStore.loading" class="d-flex h-100 flex-column">
    <BContainer fluid class="h-100">
      <BRow class="h-100 d-flex justify-content-center">
        <div id="predictionsCarosuel" v-if="!cStore.loading"
          :class="{ Overflow: cStore.CurrentPredictionCrops.length > 2 }">
          <div v-for="predCrop in cStore.CurrentPredictionCrops" :key="predCrop.uuid"
            ...
            <button @click="predCrop.approved = (predCrop.approved) ? false : true" tabindex="-1">
              <canvas :ref="(el) => { if (el) { predCropRefs[predCrop.uuid] = el as HTMLCanvasElement } }"
                :class="{ Visible: predCrop.drawBox }" tabindex="-1"></canvas>
    ...
    <BButtonToolbar key-nav justify aria-label="Auto Cropper Controls" class="bg-body-secondary w-100 mt-1">
      <BButtonGroup>
