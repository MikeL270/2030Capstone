  ...
  methods: {
    ...
    decodeDigit(event: KeyboardEvent) {
      ...
      if (this.cvs.selectedShapeName != '') {
        this.cvs.updateLabel(this.cvs.selectedShapeName, label);
      } else {
  ...
  <div v-if="cvs.bootStrapped && !cvs.loading" class="d-flex h-100 flex-column">
          ...
          <BListGroup class="overflow-y-auto overflow-x-hidden" style="max-height: 90% !important">
            <BListGroupItem v-for="label in pStore.SortedLabels"
              ...
              <div class="d-flex justify-content-between w-100 align-items-center">
                <h4 :style="{ color: label.color, borderColor: label.color }" class="label">
                  {{ label.label }}
                </h4>
              ...
              <BListGroup class="overflow-y-auto mt-2">
                <BListGroupItem v-for="annot in cvs.annotationsByLabelId(label.label_id)" class="p-2"
                  :class="{ hovered: annot.uuid === cvs.hoveredUuid }" @mouseover="(e: MouseEvent) => {
                    //@ts-ignore
    ...
    <BButtonToolbar key-nav justify aria-label="Crop Explorer Controls" class="bg-body-secondary mt-3">
      ...
      <BButtonGroup>
        ...
        <BButton class="explorerButton" @click="handleEnter()" variant="success">
